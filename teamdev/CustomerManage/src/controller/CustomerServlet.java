...
public class CustomerServlet extends BaseServlet {
    ...
    private void procUpdate(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        //セッションで来た顧客情報を「customer」に入れる　→　Bean！
        CustomerBean customer = (CustomerBean) session.getAttribute("customer");

        //岡本さんのupdateに↑のBeanを入れるとメッセージ（「null」か「何か」）が返ってくるので
        //それを「errMessage」に入れておく
    ...
    private void procAdd(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        //岡本さんのaddに↑のBeanを入れるとメッセージ（「null」か「何か」）が返ってくるので
        //それを「errMessage」に入れておく
        String errMessage = null;
    ...
    private void procDeleteConfirm(HttpServletRequest request, HttpServletResponse response, String id)
        ...
        //もらったid（文字列）をint型に変換してintCustomerIdとする
        int intCustomerId = Integer.parseInt(id);

        //CustomerLogicを新しく作る
        CustomerLogic customerLogic = new CustomerLogic();

        //新しいBeanにidが一致する顧客データを入れる
        CustomerBean customer = null;
        customer = customerLogic.load(intCustomerId);

        HttpSession session = request.getSession();

        //顧客データをセッションにいれて確認画面へ
        session.setAttribute("customer", customer);
